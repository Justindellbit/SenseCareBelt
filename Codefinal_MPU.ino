...
//angle beta
 ...
 f_module=(module<MODULE_TH);
  // Decision ( votes=facteur indicateur de chute base sur la confirmation de 6 parametres)
...
// pour Debugage
   Serial.printf("pitch=%.1f roll=%.1f alim=%.2f angle=%.1f module=%.2f g_angle=%.1f votes=%d\n",
      pitch,roll, alim, angle_deg,module,g_angle, votes);
  delay(3000);
}

bool Post_Confirmation()  //FONCTION DE COMFIRMATION DE CHUTE QUI fait un check pendant 8 seconde
{
    int cpt=0;
    while(votes >= 3 && cpt<8)
     {
